...
export const LoginPage = () => {
	...
	// const { logout, isSuccessSignout: isSuccessSignout } = useAuthContext();
	...
	const formik = useFormik({
		...
		validationSchema: Yup.object({
			...
			password: Yup.string().min(6, "password minimal 8 karakter").required("Password harus diisi"),
		}),
	...
	return (
		<AuthLayout>
						...
						InputProps={{
							...
							endAdornment: (
								<InputAdornment position="end">
									<IconButton
										aria-label={showPassword ? "Button tutup password" : "Button tampilkan password"}
			...
			{errorLogin && (
				<FailAlert
					message={errorLogin?.response?.data?.message || errorLogin?.message}
					title="Login Gagal"
				/>
			)}
			{isSuccessLogin && <SuccesAlert message="" title="Login Berhasil" />}
